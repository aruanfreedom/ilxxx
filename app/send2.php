...
if(isset($_POST["action"])):
    if($_POST["action"]=="sendform"):
        ...
        if(mb_strlen($phone,"utf-8")<=0):
            $e1.="1";
                        # это сообщение будет выведено в блоке div с id="error"
            print "<li>Заполните поле 'Номер телефона'</li>";
        endif;

        $mess=trim($_POST["message2"]);
        ...
        if($e1==null):
            ...
            $mail="info@ilx.kz"; // e-mail куда уйдет письмо
            $title="Заявка на звонок"; // заголовок(тема) письма
            //конвертируем 
            ...
            $('#myform2').trigger('reset'); /* очищаем форму */
